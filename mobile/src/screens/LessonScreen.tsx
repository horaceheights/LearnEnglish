...
import { useLessonAudioReadiness } from '../hooks/useLessonAudioReadiness';
...
export function LessonScreen({
  ...
  const preloadCardAudio = useCallback((card?: LessonCard) => {
    ...
    const isPronunciationCard = card.stage === 'Pronunciation Practice' || card.stage === 'Speak';
    const modelText = card.audio_text?.trim() || card.prompt?.trim() || '';
    const modelTurns = findCourseAudioTurnSequence(card, 'prompt');
    if (
      isPronunciationCard
      && !modelTurns
      && !findCourseAudioAsset(card, 'prompt', 'pronunciation_slow', 'split-ing', modelText)
    ) {
      addDiagnosticBreadcrumb('pronunciation_audio_asset_missing');
      return Promise.resolve(false);
    }
  ...
  const playAudioSequence = useCallback((
    sequence: CourseAudioTurnPlayback[],
    mode = 'prompt',
    ...
    void Promise.all([
      ...
      captureDiagnosticError(
        playbackError,
        'course_audio_sequence_playback',
        { mode, variant },
        'warning',
      );
    });
  ...
  const playAudioSource = useCallback((source: AudioSource, mode = 'prompt', variant = 'default') => {
    ...
    try {
      ...
      // A previous sequence may already have completed.
    }
  ...
  const playAnswerAfterChime = useCallback((text: string) => {
    ...
    answerAudioTimerRef.current = setTimeout(() => {
      ...
      const card = lesson?.cards[cardIndex];
      if (card?.answer_audio_turns?.length) {
        const sequence = findCourseAudioTurnSequence(card, 'answer');
        if (sequence) {
          playAudioSequence(sequence, 'prompt', 'answer-turns');
        } else {
          addDiagnosticBreadcrumb('course_audio_turn_sequence_invalid', { purpose: 'answer' });
        }
        return;
      }
  ...
  const load = async () => {
    ...
    try {
      ...
      setMissionKickoffComplete(nextCardIndex > 0 || Boolean(savedRun?.completionPending));
  ...
  const missionCueAsset = currentCard
    ...
    : null;
  const showMissionKickoff = Boolean(
    lesson?.experience_type === 'mission'
    && cardIndex === 0
    && completedCards.size === 0
    && !missionKickoffComplete,
  );
  const cardAudio = useLessonAudioReadiness(
    `${lessonId}:${cardIndex}:${cardRunId}`,
    currentCard?.audio_assets,
    isAppActive && !isComplete && !isCompletedSectionPicker,
    isOffline,
  );
  cardAudioReadyRef.current = cardAudio.ready;
  ...
  const isTheyTranslationCard = lesson?.id === 'lesson-3-two-people'
  ...
  const pronunciationModelAsset = isPronunciation && currentCard
    ? findCourseAudioAsset(
      ...
      'pronunciation_slow',
      'split-ing',
      pronunciationModelText,
    )
    : null;
  const pronunciationAudioIdentity = pronunciationTurnSequence?.map(({ asset }) => asset.id).join(',')
    || pronunciationModelAsset?.id
    || 'missing';
  const audioProvider = 'persistent-asset' as const;
  const audioVoice = courseAudioAssetVoice(
    pronunciationModelAsset?.id ?? `missing-pronunciation-${lessonId}-${cardIndex}`,
  );
  ...
  const promptInteractionMode = useCompactHeaderInstruction
    ? 'visual-instruction' as const
    : 'translation-on-tap' as const;
  ...
  useEffect(() => {
    ...
    const turn = activeAudioSequence[audioPlaylistStatus.currentIndex];
  ...
  }, [cardAudio.ready, cardIndex, currentCard, isOffline, preloadCardAudio, pronunciationAudioGateKey]);
  ...
  }, [cardAudio.ready, cardIndex, isComplete, isOffline, isPronunciation, onExit]);
  ...
  useEffect(() => {
    if (!showMissionKickoff || !isAppActive || !cardAudio.ready) return undefined;
    playMissionSound('mission-start');
    const timer = setTimeout(playMissionIntro, 420);
    return () => clearTimeout(timer);
  }, [cardAudio.ready, isAppActive, playMissionIntro, playMissionSound, showMissionKickoff]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      } else if (completionPromptSource) {
        playAudioSource(completionPromptSource, 'prompt', 'completion-prompt');
      } else {
        playAudio(
          promptAudio,
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!courseAudioPlaybackStatus.didJustFinish || !promptAutoplayWasPlayingRef.current)
    ) return;
  ...
  useEffect(() => {
    ...
    if (courseAudioPlaybackStatus.playing) singleCardAudioWasPlayingRef.current = true;
    if (courseAudioPlaybackStatus.error || (courseAudioPlaybackStatus.didJustFinish && singleCardAudioWasPlayingRef.current)) {
  ...
  useEffect(() => {
    if (!isAppActive || !cardAudio.ready || !isAutomaticSingleCard) return undefined;
    singleCardFallbackTimerRef.current = setTimeout(completeAutomaticSingleCard, COURSE_AUDIO_FALLBACK_MS);
    return () => {
      if (singleCardFallbackTimerRef.current) clearTimeout(singleCardFallbackTimerRef.current);
      singleCardFallbackTimerRef.current = null;
    };
  }, [cardAudio.ready, cardIndex, completeAutomaticSingleCard, isAppActive, isAutomaticSingleCard]);
  ...
  useEffect(() => {
    ...
    if (courseAudioPlaybackStatus.playing) answerAudioWasPlayingRef.current = true;
    if (!courseAudioPlaybackStatus.didJustFinish || !answerAudioWasPlayingRef.current) return;
  ...
  useEffect(() => {
    ...
    if (courseAudioPlaybackStatus.playing) grammarAnswerWasPlayingRef.current = true;
    if (!courseAudioPlaybackStatus.didJustFinish || !grammarAnswerWasPlayingRef.current) return;
  ...
  const choose = (optionId: string) => {
    ...
    const continuingSelection = isMultiBlankCompletion && result !== 'wrong' ? selectedIds : [];
    if (isMultiBlankCompletion && continuingSelection.includes(optionId)) return;
    const nextSelectedIds = isMultiBlankCompletion ? [...continuingSelection, optionId] : [optionId];
    if (missionExperience && isMissionTileCard) playMissionSound('tile-place');
    evaluateChoiceSelection(nextSelectedIds);
  };

  const submitMissionSelection = (optionIds: string[]) => {
  ...
  const recordMissionMisstep = (optionIds: string[]) => {
    ...
    if (attempt.shouldRecordAttempt) {
      attemptedCardsRef.current = attempt.attemptedCards;
      setAttemptedCards(attempt.attemptedCards);
      recordAttempt(selectedOptionKey, correctOptionKey, false, attempt.firstTry);
    }
    if (!attempt.reviewingCompletedCard) setWrongCards((current) => new Set(current).add(cardIndex));
    addDiagnosticBreadcrumb('mission_misstep_repaired', {
      card_number: cardIndex + 1,
      option_id: selectedOptionKey,
    });
    playMissionSound('try-again');
...
const styles = StyleSheet.create({
  ...
  contentHeaderPhraseBox: { justifyContent: 'center', minHeight: 76, overflow: 'visible' },
  contentHeaderPhraseBoxCompact: { minHeight: 58, paddingBottom: 4, paddingTop: 4 },
  ...
  phraseReplayIconPlaying: { backgroundColor: '#176b5d' },
  phraseReplayIconDisabled: { opacity: 0.45 },
