...
for (const filename of fs.readdirSync(generatedRoot)) {
  ...
  for (const lesson of lessons) for (const card of lesson.cards) {
    ...
    for (const asset of card.audio_assets) {
      ...
      const promptTurnMatch = asset.purpose.match(/^prompt-turn-(\d+)$/);
      const answerTurnMatch = asset.purpose.match(/^answer-turn-(\d+)$/);
      const authoredTurn = promptTurnMatch
        ? card.audio_turns?.[Number(promptTurnMatch[1]) - 1]
        : answerTurnMatch
...
for (const slideId of ['L7', 'L8', 'R6', 'S6']) {
  ...
  const turnField = card.audio_turns?.length ? 'audio_turns' : 'answer_audio_turns';
  const turnPurpose = turnField === 'audio_turns' ? 'prompt' : 'answer';
  const turns = card[turnField];
  assert.equal(turns?.length, 2, `Lesson 3.1 ${slideId} must remain a two-speaker exchange.`);
  assert.ok(
    !card.audio_assets.some((asset) => asset.purpose === turnPurpose),
    `Lesson 3.1 ${slideId} must not retain a mixed-speaker aggregate ${turnPurpose} clip.`,
  );
  for (const [turnIndex, turn] of turns.entries()) {
    const asset = card.audio_assets.find(
      (candidate) => candidate.purpose === `${turnPurpose}-turn-${turnIndex + 1}`,
    );
    assert.ok(asset, `Lesson 3.1 ${slideId} is missing ${turnPurpose} turn ${turnIndex + 1}.`);
    assert.equal(asset.text, turn.text, `Lesson 3.1 ${slideId} turn text drifted.`);
    assert.equal(asset.speaker_role, turn.speaker_role, `Lesson 3.1 ${slideId} turn voice drifted.`);
    assert.equal(asset.image_ref, turn.image_url, `Lesson 3.1 ${slideId} turn image drifted.`);
  }
}
