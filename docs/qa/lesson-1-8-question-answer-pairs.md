...
The final candidate must export its immutable audio catalog from its exact committed lesson payload, pass all backend and Preview gates, merge into `main`, and publish the exact protected `main` head only through GitHub Actions. Device testing remains the final human review step after Preview publication.
