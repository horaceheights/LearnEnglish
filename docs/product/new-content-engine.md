...
| [Unit 7 builder](../../scripts/build_unit_7_parity.py) | Existing unit-specific assembly (the Unit 1–3 builders are retired, see [the harvest](content-engine-harvest.md)); inspect for reusable patterns, not as templates for proliferating custom programs. |
...
| [Authoring layer](../../scripts/content_engine/author.py) | `scripts/content_engine_author.py BRIEF --out DIR` turns a brief (ordered items: English, Spanish, picture, kind; plus a pool of already-taught items) into a draft 40–42-card standard lesson and a review sheet. It proposes wrong options of the same kind and form that pass the shared answer-bank check. Proposals are always drafts: a person reviews every bank via [answer-choice review](../qa/answer-choice-review.md) before removing `draft` and installing. Worked example: [Family Actions brief](content-briefs/example-1.6-family-actions.json); real briefs: [Unit 1 family lessons](content-briefs/unit-1/). Before proposing, the engine asks the app's own mistake-hint resolver (`mobile/scripts/mistake-hint-oracle.cjs`) and replaces any wrong option that would only get a generic hint; a photo with an action video but no reviewed poster never goes on a two-choice picture card. `extend_lesson` appends practice cards to an existing lesson in its own style (captions, slide-id prefixes) without changing any reviewed card. Learn holds only new vocabulary: an item marked `"learn": false` is a practice-only sentence on a known frame (`It is number eight.`, `Two cars.`) and gets no Learn card; `"four_card": false` marks a picture whose answer does not survive the centered 4:5 crop, so it never appears in a four-picture card. `"choices": false` keeps an item (a Learn word or question with no wrong option of its own) out of Recognize and Listen; `"image_choices": false` offers an item only as text choices. An item with `question`/`question_es` is a reply: its picture-to-sentence cards ask that question (`What number is it?`) and play the chosen answer. A brief needs at least two new items, because the app introduces help on the second Learn card. Voices and forms: `speaker` names who says an item (a cast role such as `ana` or `male-character`); `turns` splits an exchange into lines (`[{"speaker": "sofia", "text": "Is it yours?"}, …]`, which must say exactly the item text) and keeps it out of Use; the brief's `narrators` list (`["teacher", "co-teacher"]`) alternates neutral narration card by card. `text_choices: false` keeps an item out of text answer banks (`A cook.` reads as a verb); `accepts` records a reviewed reason why a bank may hold an option the shared check would otherwise reject (`{"conflict": "reason"}`); `form` (`"word"` or `"sentence"`) overrides the guessed form (`The United States`); `use: false` keeps a fragment (`Eleven years old.`) out of Use because it has no sentence order to teach. `learn_revision` keeps a bumped audio revision on the item's Learn card when an earlier take was rejected (`Hello.` in 3.1 is revision 2), so the rejected asset never returns from a cache. New photos for a brief are generated with `scripts/render_gemini_stills.py` from a versioned pack the user has approved (dry-run by default; one receipted attempt per photo, no automatic retry, a batch ceiling, and edits only from an inspected base photo). |
