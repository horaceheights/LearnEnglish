...
| `scripts/build_unit_3_parity.py` | 2026-09-25, Unit 3 rebuild | Engine briefs in `docs/product/content-briefs/unit-3/` for the 12 teaching lessons; the review and mission are authored in their canonical files. Its three safety rules (paid receipts, hash-bound inspections, byte-identical installs) are enforced by `scripts/install_gemini_pack.py` and the preservation audit, and its fixed 3.9 counts by `docs/product/content-standards.json`. `unit-3-lesson-pack.json`, `unit-3-review-pack.json` and `unit-3-mission-pack.json` stay as reviewed data. |
