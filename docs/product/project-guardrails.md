...
- Web and mobile must resolve the same semantic and speaker roles for the same canonical card. The approved A1 character cast keeps neutral teacher, question, and answer narration in Nichalia; Ana always speaks with Sarah; and a pictured or otherwise explicitly known male character speaking the authored line uses Brian. Apply a named character voice only when the canonical media or authored dialogue makes the speaker visible or unambiguous. An object-only image, a person described in the third person, or a merely pictured person is not proof that the person is speaking. Mixed-speaker, off-screen, or otherwise ambiguous lines remain on the neutral teacher voice unless the content is later segmented and reviewed speaker by speaker.
