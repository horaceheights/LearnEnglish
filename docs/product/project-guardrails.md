...
- Internal Preview is an approved human-review surface, not proof of approval. A current semantic contract whose decision is `pending` may enter the protected Preview channel as an explicit warning so the reviewer can test the real app on a device. This exception applies only to pending human decisions: missing or malformed registry data, missing contracts or assets, stale semantic-contract or asset-binding hashes, byte-parity failures, ambiguous answers, structurally invalid media, and any `rejected` decision still block Preview. Pending four-card crop-manifest differences follow the narrower exception below. Preview testing never changes a decision automatically and never satisfies the Production approval requirement.
