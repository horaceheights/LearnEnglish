...
- On every mobile Listen card whose authored stage and prompt are exactly `Listen` and `Listen and choose.`, show one compact instruction line as bold 14 dp `¡Escucha y elige!`, keep the icon-only replay control, balance its 52 dp reserved space on both sides so the instruction is centered in the full header, and use 6 dp vertical header padding. Apply this shared rule from stage-plus-prompt identity across all seven units; never key it to a lesson, card index, option count, or image identity. The shorter header returns measured height to the existing choice layout without changing option count, arrangement, media aspect ratio, feedback reserve, or image-fit policy.
- On every mobile Recognize card with an empty authored prompt, replace the otherwise missing third line with the compact bold 14 dp instruction `¡Elige la frase correcta!`. Apply it from the Recognize-plus-empty-prompt contract wherever that interaction occurs; never key it to a lesson, card index, answer count, option copy, or image identity. Do not add a replay control because this is a visual choice instruction rather than authored prompt audio.
- On every mobile Speak card, show the compact bold 14 dp instruction `¡Escucha y repite!` in place of `Ahora escucha y repite.` or `Pronunciation Practice`, with complete Spanish exclamation punctuation in every unit. Apply the rule from the Speak interaction itself, including the legacy `Pronunciation Practice` stage when encountered; never key it to a lesson, card index, model phrase, or media identity. The model phrase remains in the pronunciation card below the header, and the active middle section label remains the shared Speak teal.
- The compact Listen instruction is not a general third-line restyle. When the third line is authored learning content—a vocabulary item, sentence, question, completion, or other teaching phrase—preserve its exact copy, established responsive type scale, and header geometry. Matching the middle section label to its journey-segment color does not authorize changing the third line.
- Authored third-line lesson text must start at its established responsive size and use the native largest-text-that-fits behavior within the available header width and two-line limit. Reduce the rendered size only as much as the actual device width, font scaling, and copy require; never use one smaller fixed size for every phrase, and never permit clipping, overflow, or an ellipsis to replace teaching text. The approved compact Spanish instruction lines remain fixed at 14 dp because they are a separate instruction standard.
- In the Lesson 1.3 portrait `They` phrase-card pilot, keep the 44 dp replay touch target transparent and borderless so only the 28 dp teal speaker control is visible at the approved right-edge position. Do not render a white outer circle, cutout, elevation, or shadow behind it. This remains a pilot-specific treatment until the phrase-card standard is reviewed.
...
- 2026-09-01: Mobile lesson section labels standardized on the exact color of their matching journey segment from one shared palette: Learn blue, Recognize coral, Listen purple, Speak teal, and Use gold. Instruction-only Listen headers across all seven units use one centered bold 14 dp `¡Escucha y elige!` line with balanced replay spacing; empty-prompt Recognize cards use `¡Elige la frase correcta!` without replay; Speak cards use `¡Escucha y repite!`; authored learning phrases retain their exact content and responsive starting size, then shrink only as much as needed to fit fully within two lines without clipping or ellipsis.
