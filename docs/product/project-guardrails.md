...
- Internal Preview is an approved human-review surface, not proof of Production approval. Agent-assisted visual QA may authorize a candidate to enter the protected Preview channel with current semantic decisions still `pending`, because Preview exists to expose the exact runtime framing for device testing. Pending decisions remain prominent warnings and are never changed automatically. This exception applies only to pending human decisions: missing or malformed registry data, missing contracts or assets, stale semantic-contract or asset-binding hashes, byte-parity failures, ambiguous answers, structurally invalid media, and any `rejected` decision still block Preview. Pending four-card crop-manifest differences follow the narrower exception below. Production still requires a person to visually approve the exact tested Preview and every bound semantic contract.
...
- Preview preflight may carry current `pending` still-image semantic and crop approvals as prominent warnings so device testing can supply the human evidence. It must continue to fail on rejected reviews, missing or malformed contracts/assets, stale or mismatched hashes, broken copy parity, ambiguous answers, structural defects, course-integrity failures, or stale release ancestry.
