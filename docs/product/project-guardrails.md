...
- If reverse-transcription or listening review shows that one deterministic short-word take is ambiguous, replace only its exact `text + mode + variant + narrator` key with a reviewed bundled take. Keep other stages and narrators unchanged, pin the approved asset in automated QA, and never apply a text-only override that silently replaces every semantic role.
...
- 2026-08-28: Ambiguous deterministic short-word audio standardized on exact-key bundled replacements after reverse-transcription and listening review; the approved take is hash-pinned and other modes, stages, and narrators remain unchanged.
