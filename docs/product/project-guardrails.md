...
- After a correct noun-to-pronoun substitution completion repeats the same predicate on both sides of a comma, apply the same gold concept emphasis to the inserted pronoun and the meaningful antecedent word or words (`woman` and `she`, `boy` and `he`, or a plural noun phrase and `they`). Detect this from the grammatical equivalence pattern, not from lesson IDs, card indexes, fixed person-word lists, or media identity. Do not color articles or conjunctions, do not reveal the relationship before completion, and do not extend the rule to unrelated completion sentences.
...
- Four-image portrait choices remain a 2x2 grid and use a taller, height-aware media viewport so the grid fills the usable lesson area. Preserve each source image's aspect ratio and complete teaching subject; do not stretch, distort, or convert the grid to another layout. This exception does not change one-, two-, or three-card layouts.
...
- One-, two-, and three-card option-media viewports use the established 3:2 shape. Four-image portrait grids use the approved taller, height-aware viewport so the fixed 2x2 grid uses the available lesson area; their reviewed 3:2 sources may crop only expendable outer background while keeping every complete subject in the central safe area.
...
- 2026-09-01: Completed noun-to-pronoun equivalence prompts standardized on shared gold emphasis for both the meaningful antecedent word or words and the inserted pronoun. The rule is derived from a repeated-predicate completion pattern and appears only after completion, so it neither hardcodes the four initial Unit 1 examples nor reveals the relationship beforehand.
