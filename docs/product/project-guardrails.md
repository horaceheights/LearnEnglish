...
- One-, two-, and three-card option-media viewports use the established 3:2 shape. Four-image portrait grids use the approved taller, height-aware viewport so the fixed 2x2 grid uses the available lesson area; the source image keeps its native aspect ratio and complete subject without stretching or distortion.
...
- On mobile, exact 3:2 option images and reviewed 3:2 variants fill the shared viewport edge-to-edge. Legacy stills with any other ratio use subject-preserving `contain` over the shared warm-neutral background (`#f2ebde`) until a reviewed 3:2 reframe or outpaint exists. Never use a generic top or center crop that removes a head, face, hand, foot, body, or teaching action.
...
- Preserve the whole teaching subject. When square or portrait source framing cannot fill 16:9 without cropping important content, center it over the shared solid warm-neutral card background (`#f2ebde`). Do not use black bars or blurred side fill. The reviewed parents-talking pilot is the sole exception: its matched 3:2 poster and top-anchored 4:3 action crop compensate for player overscan and may trim non-teaching lower-body/background area only while both heads, faces, and complete conversational hand gestures remain visible; do not generalize this exception without another visual review.
