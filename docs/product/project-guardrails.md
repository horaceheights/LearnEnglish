...
- Every published A1 option image resolves to an exact reviewed 3:2 source or 1536x1024 variant and fills its clipped viewport edge-to-edge on mobile and web. `contain` is only a runtime safety fallback for an unexpected, unmapped legacy asset; the automated all-unit guardrail must report zero published option images relying on that fallback. Never expose internal padding, black or blurred side fill, or use a generic crop that removes a head, face, hand, foot, body, or teaching action.
...
- 2026-08-24: All published A1 option stills standardized on reviewed 3:2 sources or 1536x1024 variants that fill every one-, two-, three-, and four-card tile edge-to-edge. Runtime `contain` remains only an unexpected-asset fail-safe, and automated QA requires zero catalog images to depend on it.
