...
def selected_assets(args: argparse.Namespace) -> list[tuple[CourseAudioAsset, LessonCard]]:
    ...
    requested_speakers = set(getattr(args, "speaker_role", None) or [])
    unknown_speakers = sorted(requested_speakers - set(SPEAKER_NARRATORS))
    if unknown_speakers:
        raise ValueError("Unknown speaker roles: " + ", ".join(unknown_speakers))
    if not (
        args.all_named_speakers
        or args.all_missing_after_reviewed_seed
        or requested_asset_ids
        or requested_lessons
        or requested_speakers
    ):
        raise ValueError(
            "Select --all-named-speakers, --all-missing-after-reviewed-seed, "
            "--lesson-id, --asset-id, or --speaker-role."
        )
        ...
        for card in lesson.cards:
            for asset in card.audio_assets:
                ...
                if requested_lessons and lesson.id not in requested_lessons:
                    continue
                if requested_speakers and asset.speaker_role not in requested_speakers:
                    continue
...
def verify_legacy_backend(client: httpx.Client, base_url: str) -> None:
    ...
    expected_voices = {
        ...
        "male-conversational": render_profile_for("luis", "prompt").voice_id,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--speaker-role",
        action="append",
        help="Select every asset for one semantic speaker role. May be repeated.",
    )
